...
export default defineConfig(({ mode }) => {
  ...
  // the SW precache conflicts with the Tauri asset/resource protocol and
  // would duplicate every font/image already shipped as native resources.
  // With `disable: true` below, `virtual:pwa-register` resolves to a noop
  // stub, so `src/main.tsx` needs no branching.
