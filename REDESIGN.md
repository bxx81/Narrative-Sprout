...
- `src-tauri/target/` のみ `.gitignore`（Rust ビルド成果物はコミットしない）
- Tauri バイナリのビルドはタグ時の専用 CI ワークフローで行い、通常の PR CI（lint / tsc / test）は Web のみを対象とする（Rust ビルドは含めない）
...
interface GameRecord {
  ...
  // 生成設定は原則保持しない（§5.4。グローバル settings を常時参照）。例外は sceneTextLength のみ（セーブ作成時にスナップショット）
  sceneTextLength?: string; // 作成時の本文長オーダー（legacy 準拠）。以後このセーブの生成はこの値を使う。旧セーブは欠損 → グローバル settings にフォールバック
...
- **例外: `sceneTextLength` はセーブ作成時に `GameRecord` へスナップショットする**（実機確認で判明した UX 差。現行版は per-save で、800-1600 語で作ったセーブが後から設定を変えても長さを保つべきで、グローバル常時参照だと読み込み時の現在設定で生成されてしまう）。以後このセーブの生成はスナップショット値を使い、旧セーブ（欠損）はグローバル設定にフォールバック。他の生成設定（モデル・画像等）はグローバル一本のまま
