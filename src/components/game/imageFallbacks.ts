...
/**
 * Shared `<img onError>` handler for every card / chronicle thumbnail: a
 * stored asset that fails to decode degrades to the same "Image Not
 * Available" placeholder as a node with no asset at all. The source check
 * keeps a failing fallback from looping.
 */
export function applyLoadScreenFallback(event: SyntheticEvent<HTMLImageElement, Event>): void {
  const image = event.currentTarget;
  if (image.src !== LOAD_SCREEN_FALLBACK_URL) {
    image.src = LOAD_SCREEN_FALLBACK_URL;
  }
}
