...
| react-router | 7.x | Route-based screens + `RequireActiveGame` guards. |
...
| i18next / react-i18next | 25.x / 16.x | UI localization with bundled resources (no http-backend). |
...
| Package | Purpose |
|---------|---------|
| typescript | 5.x, strict (`tsc --noEmit` in build + CI). |
