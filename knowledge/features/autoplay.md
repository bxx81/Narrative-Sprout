...
- A decision-phase `AbortError` settles to `autoplayTurn: idle` + `autoplay: false` silently (unlike `generation` aborts which show the informational `errorAborted` dialog). Genuine decision errors land in `autoplayTurn: failed` + `autoplay: false` and **do** open the global `ErrorDialog`, dismiss-only — there is no decision payload for `retryGeneration`, so the user fixes the cause and toggles autoplay back on.
